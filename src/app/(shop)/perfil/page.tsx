...
// Se eliminaron imports de orders, types y formatPrice
...
const [activeTab, setActiveTab] = useState<"profile" | "account">("profile");
  ...
  const handleUpdateProfile = async () => {
    ...
    if (!validation.success) {
      setToast({
        show: true,
        msg: validation.error || "Error en el formulario",
        type: "error",
      });
      return;
    }
    ...
    try {
      ...
      if (profile.profilePhoto.includes("firebasestorage")) {
        ...
        await deleteObject(storageRef).catch((err) =>
          console.warn("No se pudo borrar de storage o no existía", err),
        );
      }
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      if (
        ...
        profile.profilePhoto.includes("firebasestorage")
      ) {
        const storageRef = ref(storage, `users/${user.uid}/profile-photo`);
      ...
      if (error.code === "auth/requires-recent-login") {
        setToast({
          show: true,
          msg: "Por seguridad, iniciá sesión nuevamente para eliminar tu cuenta.",
          type: "error",
        });
        ...
        setToast({
          show: true,
          msg: "Error al eliminar la cuenta. Contactá a soporte.",
          type: "error",
        });
      }
    }
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 flex flex-col md:flex-row items-center gap-8">
            ...
            <div className="relative group shrink-0">
              <div className="relative h-24 w-24 shrink-0 overflow-hidden rounded-full border-4 border-indigo-50 bg-indigo-50">
                {profile.profilePhoto ? (
<img
    // CORRECCIÓN 1: Si es null, usa string vacío
    src={profile.profilePhoto || ""} 
    
    // CORRECCIÓN 2: Si el nombre es null, usa "Avatar" o vacío
  ...
  />
                  ...
                  <div className="flex flex-wrap gap-2 justify-center md:justify-start mt-2">
                    {profile.role === "admin" ? (
                      ...
                      <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-green-100 text-green-700 border border-green-200">
                        <CheckCircle className="h-3 w-3 mr-1" /> Cliente
                        Verificado
                      </span>
                    )}
            ...
            <div className="flex flex-col sm:flex-row gap-3 items-center">
              ...
              <button
                onClick={() =>
                  auth.signOut().then(() => router.push("/tienda"))
                }
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                >
                  <div className="p-6 border-b border-gray-50 flex justify-between items-center">
                    <h2 className="text-lg font-black text-gray-900 flex items-center gap-2">
                      <UserIcon className="h-5 w-5 text-indigo-600" /> Datos
                      Personales
                    </h2>
                    {!isEditing && (
                      <button
                ...
                >
                  <div className="p-6 border-b border-gray-50">
                    <h2 className="text-lg font-black text-gray-900 flex items-center gap-2">
                      <MapPin className="h-5 w-5 text-green-600" /> Mis
                      Direcciones
                    </h2>
                  </div>
                  <div className="p-6">
                    ...
                    <p className="text-xs text-gray-400 mt-2 flex items-center gap-1">
                      <Shield className="h-3 w-3" /> Esta dirección se usará
                ...
                >
                  ...
                  <div className="p-6 space-y-4">
                    <p className="text-sm text-gray-600">
                      ¿Tenés algún problema con tu pedido o cuenta?
                    </p>
                    <a
                      href="mailto:soporte@mitienda.com"
                      className="flex items-center gap-3 p-3 rounded-xl bg-orange-50 text-orange-700 font-bold hover:bg-orange-100 transition-colors"
                    >
                      <Mail className="h-5 w-5" /> Contactar Soporte
                    </a>
                    <a
                      href="#"
                      className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 text-gray-700 font-bold hover:bg-gray-100 transition-colors"
                    >
                ...
                <div className="bg-red-50 rounded-3xl border border-red-100 overflow-hidden">
                  <div className="p-6">
                    ...
                    <p className="text-sm text-red-700 mb-4">
                      Si eliminás tu cuenta, perderás todo tu historial de
                      pedidos y favoritos. Esta acción no se puede deshacer.
                    </p>
